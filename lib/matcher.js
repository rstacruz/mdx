...
Matcher.prototype = {
  ...
  partialFromMany (def) {
    if (!def.separator) throw new Error('No separator')
  ...
  match (id, str) {
    ...
    if (matcher.indices.length > 0) {
      matcher.indices.forEach(function (name, i) {
        output[name] = m[i + 1]
      })
    }
...
function escapeGroups (regexp) {
  return regexp.replace(/^\(|[^\\]\((?!\?:)/g, function (match) {
    return match + '?:'
  })
}
